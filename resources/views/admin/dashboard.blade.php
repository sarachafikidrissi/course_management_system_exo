                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($user->roles[0]->name == 'teacher')
                                            <form action="/teacher/approve/{{ $user->id }}" method="post">
                                            @csrf
                                            @method('PUT')
                                            <input type="text" name="user_id" value="{{ $user->id }}" class="hidden">
                                            <button
                                            class="px-4 py-2 inline-flex text-xs leading-5 font-semibold rounded-full text-white" style="background: {{ $user->approved ? 'red' : 'green' }};"> {{ $user->approved ? 'Approved' : 'Not Approved' }}</button>
                                        </form>
                                        @endif
